	...
	for line in tweetFile:
		...
		if 'user' in tweet and 'location' in tweet['user']:
			if tweet['user']['location'] is not None:
				locationToken = tweet['user']['location'].encode('utf-8').split(", ")

				if len(locationToken) == 2:
					if stateSentiment.has_key(locationToken[1]):
						stateSentiment[locationToken[1]] += sentimentScore
			else:
				continue

	happinestState = sorted(stateSentiment.items(), key = operator.itemgetter(1), reverse = True)[0]
	stateAbbr, stateName = happinestState
	print stateAbbr
